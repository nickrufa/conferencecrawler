<cfquery name="getAllECCMIDSessionData" datasource="#application.dsn#">
    SELECT id, parsedSessionData, faculty
    FROM ECCMID_2024
    WHERE 0=0
    ORDER BY id 
</cfquery>
<cfif 0><cfdump var="#getAllECCMIDSessionData#" label="getAllECCMIDSessionData"></cfif>
<cfloop query="getAllECCMIDSessionData">
   
    <cfif not len(trim(getAllECCMIDSessionData.faculty))>
        <cfset cleanSessionData = trim(getAllECCMIDSessionData.parsedSessionData)>

        <cfoutput>
            <!--- cleanSessionData: #cleanSessionData#<hr> --->
