...
for url, file_id in zip(urls, range(265, 530)):
  ...
  for item in presentation_list:
      ...
      presenter_parts = presenter_container.find_all(string=True, recursive=False) if presenter_container else []
      presenter = ' '.join([part.strip() for part in presenter_parts if part.strip()]) if presenter_container else 'N/A'
