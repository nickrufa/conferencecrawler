...
def extract_data_from_html(html_content, url):
    ...
    print_html_snippet(decoded_html)
    ...
    if content_div:
        ...
        if title_elem:
            ...
            logging.info(f"Title found: {data['title']}")
        else:
        ...
        if info_div:
            ...
            if len(lines) >= 2:
                ...
                logging.info(f"Track and session found for {url}")
            else:
                logging.warning(f"Unexpected structure in info div for {url}")
        else:
            logging.warning(f"No info div found for {url}")
        ...
        if tidbit_elems:
            ...
            if len(tidbit_elems) > 1:
                ...
                if len(times) == 2:
                    data['starttime'] = times[0].strip()
                    data['endtime'] = times[1].strip()
            data['location'] = tidbit_elems[2].text.strip() if len(tidbit_elems) > 2 else ''
            logging.info(f"Date, time, and location extracted for {url}")
        else:
            logging.warning(f"No pres-tidbit elements found for {url}")
        ...
        for elem in presenter_elems:
            ...
            presenter['details'] = ' '.join(details)
            presenters.append(presenter)
        data['presenters'] = presenters
        logging.info(f"{len(presenters)} presenters found for {url}")
    else:
