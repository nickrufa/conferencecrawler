...
def extract_poster_data(html_content):
    ...
    for poster_div in poster_divs:
        ...
        try:
            ...
            if abstract_div:
                abstract_number = abstract_div.find('p', class_='pt-2 pb-0').find('strong', text='Abstract: ').next_sibling.strip()
        ...
        try:
            ...
            session_div = poster_div.find('p', class_='pb-0', string=re.compile('Session title:'))
            if session_div:
        ...
        # Extract presenter
        try:
            presenter_div = poster_div.find('strong', text=re.compile('Poster Presenter:'))
            if presenter_div and presenter_div.parent:
                presenter = presenter_div.parent.find('p', class_='pb-0').text.strip()
        ...
        # Extract co-authors
        try:
            authors_div = poster_div.find('span', class_='font-weight-bold', text='Co-Authors:')
            if authors_div and authors_div.parent:
                authors = []
                for span in authors_div.parent.find_all('span'):
                    # Skip the "Co-Authors:" label and comma separators
                    if span.get('class') and 'font-weight-bold' in span.get('class'):
                        continue
                    if span.text.strip() == ',':
                        continue
                    if span.text.strip():
