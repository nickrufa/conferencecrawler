...
def extract_poster_data(html_content):
    ...
    for poster_div in poster_divs:
        ...
        try:
            ...
            # Find the paragraph with the session title
            for p in poster_div.find_all('p', class_='pb-0'):
                p_text = p.get_text()
                if 'Session title:' in p_text:
                    session = p_text.replace('Session title:', '').strip()
        ...
        # Extract co-authors with a direct string-based approach
        try:
            authors = []
            # First find any div with class="col-12" that contains the co-authors text
            for div in poster_div.select('div.col-12'):
                div_text = div.get_text()
                if 'Co-Authors:' in div_text:
                    # This is the div containing co-authors
                    parts = div_text.split('Co-Authors:')
                    if len(parts) > 1:
                        authors_text = parts[1].strip()
                        
                        # Split by commas and clean
                        raw_authors = [a.strip() for a in authors_text.split(',')]
...
def main():
    ...
    try:
        ...
        print(f"Successfully read HTML file: {args.input_file}")
    ...
    print(f"Extracted {len(posters)} posters from HTML")
