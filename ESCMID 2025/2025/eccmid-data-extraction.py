    ...
    try:
        ...
        if presentations_container:
            ...
            for group in presentation_groups:
                ...
                # Get presenters
                presenter_elements = group.select('.modal-session-faculties')
                for presenter in presenter_elements:
                    presenter_name = presenter.text.strip()
                    presentation['presenters'].append(presenter_name)
...
def process_all_records():
    ...
    try:
        ...
        print("1. Update the main ECC table with the extracted data")
        ...
        if choice == '1':
            # Update the main ECC table with the extracted data
            try:
                cursor.execute("""
                ...
                conn.commit()
                print(f"Successfully updated {cursor.rowcount} records in the main ECC table.")
                logging.info(f"Successfully updated {cursor.rowcount} records in the main ECC table.")
            except mysql.connector.Error as err:
